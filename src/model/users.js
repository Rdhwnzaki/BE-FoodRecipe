...
const create = (data) => {
  const { id_user, email_user, password_user, fullname_user, phone, otp } =
    data;
  return new Promise((resolve, reject) =>
    Pool.query(
      `INSERT INTO users(id_user,email_user,password_user,fullname_user,phone,verif,otp)VALUES('${id_user}','${email_user}','${password_user}','${fullname_user}','${phone}',0,'${otp}')`,
      (err, result) => {
        if (!err) {
          resolve(result);
        } else {
